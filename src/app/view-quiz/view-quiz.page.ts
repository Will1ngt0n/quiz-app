...
export class ViewQuizPage implements OnInit {
  ...
  categoryCode; userId
  index = 0
  constructor(public quizService: QuizService, public quizResultsService: QuizResultsService, public route : ActivatedRoute, public userService : UserService) {
  ...
  setScore(event, Question){
    ...
    if(this.index != null){
      ...
      //Pushiong to the game arrayxxxxxxxxxxxxxxxxxxxxxxxxxxx
      this.pushToGameArray(question, userAnswer, correctAnswer, scoreBoolean)
      }
    }
  ...
  submitScores(){
    ...
    if(this.gameArray){
      ...
      this.quizResultsService.submitToFirebase(gameScore, this.gameArray, this.categoryCode, this.userId)
